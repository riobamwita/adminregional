...
function ensureOption(select, value, label = value) {
    ...
    /* Insert right after the placeholder so it sits at the top of the list */
...
export async function initVehicleCatalogue({
    ...
    setOptions(make, rows.map(row => row.make), "Select make");

    clearDependentFields(model, year, body, fuel, trans, drive);

    make.onchange = () => {
        const selectedMake = make.value;

        const matchingRows = rows.filter(row => row.make === selectedMake);
    ...
    model.onchange = () => {
        ...
        setOptions(year, matchingRows.flatMap(row => row.years), "Select year");
        setOptions(body, matchingRows.flatMap(row => row.body_types), "Select body type");
        setOptions(fuel, matchingRows.flatMap(row => row.fuel_types), "Select fuel type");
        setOptions(trans, matchingRows.flatMap(row => row.transmissions), "Select transmission");
        setOptions(drive, matchingRows.flatMap(row => row.drive_types), "Select drive type");
    ...
    function getModelRecord() {
        return rows.find(
            row => row.make === make.value && row.model === model.value
        );
    }
