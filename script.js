...
async function load(){if(!await auth())return;$("loading").style.display="block";grid.innerHTML="";$("empty").style.display="none";$("error").classList.remove("active");try{let{data,error}=await supabase.from("cars").select("*").order("created_at",{ascending:false});if(error)throw error;cars=data||[];
stats();
syncFilters();
...
syncFilters();
render();
});$("refreshListings").onclick=load;$("menu").onclick=()=>{$("sidebar").classList.add("open");$("overlay").classList.add("show")};$("closeMenu").onclick=$("overlay").onclick=()=>{$("sidebar").classList.remove("open");$("overlay").classList.remove("show")};$("logoutBtn").onclick=async()=>{await supabase.auth.signOut();location.replace("auth.html")};window.addEventListener("load",()=>setTimeout(()=>$("loader")?.classList.add("hide"),450));document.querySelectorAll(".add-vehicle").forEach(b=>b.onclick=async e=>{if(!await canManageVehicles()){e.preventDefault();alert("Access Denied: You do not have permission to add vehicles.")}});load();